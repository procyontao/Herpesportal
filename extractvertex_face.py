...
_rlnCoordinateY #23\n")
    ...
    for item in groups:
        ...
        t = np.array([np.cos(psi),-np.sin(psi)],dtype=np.float)*np.sin(tilt)*490+720

        #_rlnImageName and _rlnMicrographName
        coord.write(j[0]+" "+coordName.split('.')[0]+".mrc ")

        #defocusUV
        coord.write(str(float(j[2])+np.cos(tilt)*504.7)+" ")
        coord.write(str(float(j[3])+np.cos(tilt)*504.7)+" ")
...
if __name__=='__main__':
    forg=open('frombin4run1_expended_face.star','r')
    fo = forg.readlines()
    i=0
    o=len(fo)
